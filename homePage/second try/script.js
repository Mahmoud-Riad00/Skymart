...
async function getData() {
    try {
        ...
        let categoriesRes = await fetch ('https://fakestoreapi.com/products/categories')
...
function scrollAllProducts() {
    ...
    let intervalTime = 100; // Adjust scroll interval time
    const scrollInterval = setInterval(() => {
        scrollX += scrollSpeed;
        products.scrollLeft = scrollX;

        if (scrollX >= products.scrollWidth - products.clientWidth) {
...
getData().then(() => {
    scrollAllProducts();  // Only scroll after products have been loaded
});
